...
<div id="moduleName"><img src="<{$xoops_url}>/<{$imgdir}>/icon/open.png" width="36" height="24" />&nbsp;<{$lang_modulename}>&nbsp;<img src="<{$xoops_url}>/<{$imgdir}>/icon/close.png" width="36" height="24" /></div>
<div id="pagePath"><a href="<{$xoops_url}>"><{$smarty.const._MD_SOAPBOX_HOME}></a> &bull; <a href="<{$xoops_url}>/modules/<{$lang_moduledirname}>/"><{$lang_modulename}></a> &bull; <a href="<{$xoops_url}>/modules/<{$lang_moduledirname}>/column.php?columnID=<{$story.columnID}>"><{$story.colname}></a> &bull; <{$story.headline}></div>

<h3 class="reddish"><{$story.colname}></h3>

<table><tbody><tr>
    <td>
        <h3><{$story.headline}></a></h3>
    </td>
    <td style="text-align: right;"><{$story.adminlinks}><{$story.userlinks}></td>
</tr></tbody></table>
...
<th style="font-size: 11px;"><{$smarty.const._MD_SOAPBOX_MOREBY}><{$story.authorname}></th>
<tr>
<td><ul style="margin: 0;">
<{foreach item=link from=$listarts.links}>
<li class="box"><a style="font-size: 10px;" href="<{$xoops_url}>/modules/<{$lang_moduledirname}>/article.php?articleID=<{$link.articleID}>"><{$link.arttitle}><br /><span class="smallpub"><{$link.published}></span></a></li>
<{/foreach}>
</ul></td>
</tr>
<tr><td><div class="pad18"><img src="<{$xoops_url}>/<{$imgdir}>/rarrow.gif" width="10" /><{$readmore}></div></td></tr>
</table>
</div>
<{/if}>

<div class="byline"><{$smarty.const._MD_SOAPBOX_BY}><{$story.author}> | <{$story.posted}></div>
...
<div class="colteaser"><{$story.colname}>: <{$story.coldesc}></div>

<div class="coldesc8"><{$smarty.const._MD_SOAPBOX_ARTREAD}><{$story.counter}><{$smarty.const._MD_SOAPBOX_TIMES}>

<{if $showrating != 0}>
    <{if $story.rating == '0.000'}>
    | <{$story.rating}>
    <{elseif $story.rating != '0.000'}>
    | <{$story.rating}> | <{$story.votes}>.</div>
    <{/if}>
<{/if}>
...
<{$authorpm_link}>
<{ /if }>
<a href="print.php?articleID=<{$story.id}>"><img src="<{xoModuleIcons16 printer.png}>" border="0" alt="<{$smarty.const._MD_SOAPBOX_PRINTERFRIENDLY}>" /></a>
<a href="<{$mail_link}>" target="_top"><img src="<{xoModuleIcons16 mail_forward.png}>" border="0" alt="<{$smarty.const._MD_SOAPBOX_SENDSTORY}>" /></a>
</div>
...
<form method="POST" action="article.php">
  ...
  <div align="center"> <br>
    <{$rate_gtickets}>
    <input type="submit" name="submit" value="<{$lang_ratethis}>">
  </div>
</form>
<{/if}>
